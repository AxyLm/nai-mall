...
export default {
    ...
    methods:{
        ...
        getPage(){
            ...
            let api = this.$http.api.comment.getPage();
        ...
        // 增加和修改操作
        clickAdd(){ 
            this.modelTitle = "轮播图 - 新建";
            this.showDetail = true;
            this.$nextTick(_=>{
                this.$refs.commentDetail.init();
            });
        },
        clickEdit(row){ 
            this.modelTitle = "轮播图 - 修改";
            this.showDetail = true;
            this.$nextTick(_=>{
                this.$refs.commentDetail.init(row.id);
            });
        },
        ...
        clickDel(row){
            this.$confirm("确定删除吗？").then(_=>{
                let api = this.$http.api.comment.delete(row.id);
        ...
        clickDelMany(){
            ...
            let api = this.$http.api.comment.batchDelete();
            this.$http.callApi(api,{
                data:{
        ...
        clickDeletedSwitch(row){
            let api = this.$http.api.comment.updateDeleted(row.id);
            this.$http.callApi(api,{
                data:{
                    deleted:row.deleted
                }
    ...
    computed:{
        rights(){
            return this.$store.state.rights.order.comment;
        }
    },
    components:{
        CommentDetail
