...
export default {
    ...
    methods:{
        ...
        getPage(){
            ...
            let api = this.$http.api.configStore.getPage();
            this.$http.callApi(api,{
                params:{pagenum, pagesize},
                data:this.filterData
            }).then(res=>{
                this.tableData = res.data.store;
        ...
        // 增加和修改操作
        clickAdd(){ 
            this.modelTitle = "门店 - 新建";
            this.showDetail = true;
            this.$nextTick(_=>{
                this.$refs.storeDetail.init();
            });
        },
        clickEdit(row){ 
            this.modelTitle = "门店 - 修改";
            this.showDetail = true;
            this.$nextTick(_=>{
                this.$refs.storeDetail.init(row.id);
            });
        },
        ...
        clickDel(row){
            this.$confirm("确定删除吗？").then(_=>{
                let api = this.$http.api.configStore.delete(row.id);
        ...
        clickDelMany(){
            ...
            let api = this.$http.api.configStore.batchDelete();
            this.$http.callApi(api,{
                data:{
        ...
        clickDeletedSwitch(row){
            let api = this.$http.api.configStore.updateDeleted(row.id);
            this.$http.callApi(api,{
                data:{
                    deleted:row.deleted
                }
    ...
    computed:{
        rights(){
            return this.$store.state.rights.system.store;
        }
    },
    components:{
        StoreDetail
