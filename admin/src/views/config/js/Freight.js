import FreightDetail from "../FreightDetail.vue";
export default {
    name: "Freight",
    data() {
        return {
    ...
    methods:{
        ...
        getPage(){
            ...
            let api = this.$http.api.freight.getPage();
        ...
        // 增加和修改操作
        clickAdd(){ 
            this.modelTitle = "运费模板 - 新建";
            this.showDetail = true;
            this.$nextTick(_=>{
                this.$refs.freightDetail.init();
            });
        },
        clickEdit(row){ 
            this.modelTitle = "运费模板 - 修改";
            this.showDetail = true;
            this.$nextTick(_=>{
                this.$refs.freightDetail.init(row.id);
            });
        },
        ...
        clickDel(row){
            this.$confirm("确定删除吗？").then(_=>{
                let api = this.$http.api.freight.delete(row.id);
        ...
        clickDelMany(){
            ...
            let api = this.$http.api.freight.batchDelete();
            this.$http.callApi(api,{
                data:{
        ...
        clickDeletedSwitch(row){
            let api = this.$http.api.freight.updateDeleted(row.id);
            this.$http.callApi(api,{
                data:{
                    deleted:row.deleted
                }
    ...
    computed:{
        rights(){
            return this.$store.state.rights.system.freight;
        }
    },
    components:{
        FreightDetail
